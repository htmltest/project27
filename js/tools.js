...
(function($) {
    ...
    $(document).ready(function() {
        ...
        $('nav a, .header-order-link, .promo-order-link').click(function(e) {
            ...
            if ($(this).hasClass('header-order-link')) {
                $('.order-tabs a').eq(1).click();
            } else {
                $('.order-tabs a').eq(0).click();
            }
            e.preventDefault();
        });
        ...
        // анонсы
        function resizeAnonces() {
            $('.anonces-list').each(function() {
                var curBlock = $(this);
                curBlock.find('.anonces-item').css({'min-height': 0});
                curBlock.find('.anonces-item:nth-child(3n)').each(function() {
                    var curItem   = $(this);
                    var curIndex  = curBlock.find('.anonces-item').index(curItem);
                    var prevItem  = curBlock.find('.anonces-item').eq(curIndex - 1);
                    var firstItem = curBlock.find('.anonces-item').eq(curIndex - 2);
                ...
                if (curBlock.find('.anonces-item').length % 3 == 2) {
                    var curItem   = curBlock.find('.anonces-item:last');
                    var curIndex  = curBlock.find('.anonces-item').index(curItem);
                    var prevItem  = curBlock.find('.anonces-item').eq(curIndex - 1);
        ...
        $(window).load(resizeAnonces);

        $('.anonces-list').on('click', '.anonces-item-name a', function(e) {
            $.ajax({
                url: $(this).attr('href'),
                dataType: 'html',
                cache: false
        ...
        $('.anonces-more a').click(function(e) {
            $.ajax({
                url: $(this).attr('href'),
                dataType: 'html',
                cache: false
            }).done(function(html) {
                $('.anonces-list').append(html);
                resizeAnonces();
            });
            $('.anonces-more').remove();
            e.preventDefault();
        ...
        // отзывы
        $('.responses-item-video a').click(function(e) {
            var curLink = $(this);
            windowOpen('<iframe width="640" height="360" src="' + curLink.attr('href') + '?wmode=transparent&autoplay=1" frameborder="0" allowfullscreen></iframe>');
            e.preventDefault();
        });

        $('.responses-item-open a').click(function(e) {
            $.ajax({
                url: $(this).attr('href'),
                dataType: 'html',
                cache: false
                ...
                windowOpen(html);
                if ($('.window-container').height() > $(window).height() - 40) {
                    $('.window-overlay').css({'min-height': $('.window-container').height() + $('.responses-window .responses-item-author').height() + 40});
                } else {
                    $('.window-overlay').css({'min-height': $('.window-container').height() + $('.responses-window .responses-item-author').height()});
                }
            });
            e.preventDefault();
        });

        $('.responses-container').each(function() {
            var curSlider = $(this);
            curSlider.data('disableAnimation', true);
        ...
        $('#responses').on('click', '.responses-ctrl a', function(e) {
            var curSlider = $('.responses-container');
        ...
        $('body').on('click', '.responses-window-close a', function(e) {
            windowClose();
            e.preventDefault();
        });
        ...
        $('.order-tabs a').click(function(e) {
            ...
            if (!curLink.hasClass('active')) {
                var curIndex = $('.order-tabs a').index(curLink);
                $('.order-tabs a').removeClass('active');
                curLink.addClass('active');
                $('.order-forms form').removeClass('active');
                $('.order-forms form').eq(curIndex).addClass('active');
            }
            e.preventDefault();
        });
